...
2. Right now, the application isn't very dynamic in that it can be difficult to add new languages and greetings without modifying the code every time. Just thinking programmatically,  how could you make the application more dynamic? You are free to reference Geeting.java and how that could be used in your design.
      ...
      It may be possible to create a database in advance, write all the parameters that might be needed into the database, and then change the project code from manual input to read from the database, so that if you need to add new elements later, you can add them to the database
